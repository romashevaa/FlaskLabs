...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and check_password_hash(user.password, form.password.data):
            ...
            jwt_token = user.generate_access_token()
            print(request)
            if request.args.get('get_token') == 'true':
                return jsonify({'jwt_token': jwt_token})
            flash('Login successful.', 'success')
            return redirect(url_for('main.index', jwt_token=jwt_token))
        else:
            ...
            if existing_email:
                ...
                if not form.validate_username(form.username):
                    ...
                    if not form.validate_email(form.email):
                        ...
                        db.session.commit()
                        jwt_token = new_user.generate_access_token()
                        if request.args.get('get_token') == 'true':
                            return jsonify({'jwt_token': jwt_token})
